...
bool sepp_n32c_handshake_send_termination(sepp_node_t *node)
{
    ...
    request = sepp_n32c_handshake_build_termination(node);
    if (!request) {
        ogs_error("sepp_n32c_handshake_build_exchange_capability() failed");
        return false;
    }
...
static int request_handler(ogs_sbi_request_t *request, void *data)
{
    int rv;
    ogs_hash_index_t *hi;
    ogs_sbi_client_t *client = NULL, *nrf_client = NULL, *next_sepp = NULL;
    ...
    for (hi = ogs_hash_first(request->http.headers);
        ...
        if (!strcasecmp(key, OGS_SBI_USER_AGENT)) {
             ...
             *
             * NOTE 3: The SEPP can assume that the service request corresponds
     ...
     ************************************/
    if (next_sepp || headers.target_apiroot || client) {
        ...
        if (next_sepp) {
            /* Switch to the Next-SEPP's client */
            client = next_sepp;
        ...
        /* Copy Request for sending SEPP */
        copy_request(&sepp_request, request, next_sepp ? true : false);
        ...
        if (ogs_sbi_client_send_request(
                    client, response_handler, &sepp_request, assoc) != true) {
        ...
        int status, ogs_sbi_response_t *response, void *data)
{
    ...
    ogs_sbi_nf_instance_t *nf_instance = NULL;
    ogs_sbi_client_t *client = NULL, *next_sepp = NULL;
    ...
    /* Copy Request for sending SEPP */
    copy_request(&sepp_request, request, false);
    ogs_assert(sepp_request.http.headers);
    ...
    if (next_sepp) {
        ...
        ogs_sbi_header_set(sepp_request.http.headers,
    ...
    /* Setup New URI */
    sepp_request.h.uri = ogs_msprintf("%s%s", apiroot, request->h.uri);
    ogs_assert(sepp_request.h.uri);
